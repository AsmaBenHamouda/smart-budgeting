...
def liste_categories(request):
    ...
    Affiche les totaux de revenus et dépenses par catégorie
    """
    ...
    # Enrichir chaque catégorie avec ses totaux
    categories_enrichies = []
    for cat in categories:
        total_revenus = cat.get_total_revenus(request.user)
        total_depenses = cat.get_total_depenses(request.user)
        solde = cat.get_solde(request.user)
        pourcentage = cat.get_pourcentage_utilise(request.user)
        ...
        categories_enrichies.append({
            ...
            'est_en_deficit': cat.est_en_deficit(request.user),
        })
    
    context = {
        'categories': categories,
        'categories_enrichies': categories_enrichies,
...
def detail_categorie(request, pk):
    """Affiche les détails d'une catégorie avec ses totaux"""
    categorie = get_object_or_404(Categorie, pk=pk)
    ...
    # Calculer les totaux
    total_revenus = categorie.get_total_revenus(request.user)
    total_depenses = categorie.get_total_depenses(request.user)
    solde = categorie.get_solde(request.user)
    pourcentage = categorie.get_pourcentage_utilise(request.user)
    
    # Récupérer les transactions récentes
    from apps.revenueApp.models import Revenue
    from apps.depenseApp.models import Depense
    
    revenus_recents = Revenue.objects.filter(
        categorie=categorie, 
        user=request.user
    ).order_by('-date')[:5]
    
    depenses_recentes = Depense.objects.filter(
        categorie=categorie,
        user=request.user
    ).order_by('-date')[:5]
    ...
    context = {
        ...
        'solde': solde,
        'pourcentage': pourcentage,
        'est_en_deficit': categorie.est_en_deficit(request.user),
