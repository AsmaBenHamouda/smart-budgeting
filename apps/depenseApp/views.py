...
class DepenseListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        # Filtrer uniquement les dépenses de l'utilisateur connecté
    ...
    def get_context_data(self, **kwargs):
        ...
        # ✅ Filtrer par utilisateur pour le total des dépenses
        context['total_depenses'] = (
            Depense.objects.filter(user=self.request.user)
            .aggregate(Sum('amount'))['amount__sum'] or 0
        )

        # ✅ Filtrer par utilisateur pour les dépenses du mois
        total_expenses_month = (
            Depense.objects.filter(user=self.request.user, date__year=year, date__month=month)
...
class DepenseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        # Un utilisateur ne peut voir que ses propres dépenses
        return Depense.objects.filter(user=self.request.user)
...
class DepenseCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # ✅ Lier la dépense à l'utilisateur connecté
        ...
        # ✅ Vérifier si la catégorie est en déficit et envoyer une alerte
        if self.object.categorie:
            from apps.categorieApp.services import verifier_et_envoyer_alerte_categorie
            alerte_envoyee = verifier_et_envoyer_alerte_categorie(
                self.object.categorie, 
                self.request.user
            )
            if alerte_envoyee:
                messages.warning(
                    self.request, 
                    f"⚠️ Attention ! Vos dépenses dans la catégorie '{self.object.categorie.nom}' "
                    f"dépassent vos revenus. Un email d'alerte vous a été envoyé."
                )
...
class DepenseUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_queryset(self):
        # Un utilisateur ne peut modifier que ses propres dépenses
        return Depense.objects.filter(user=self.request.user)
...
class DepenseDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_queryset(self):
        # Un utilisateur ne peut supprimer que ses propres dépenses
        return Depense.objects.filter(user=self.request.user)
